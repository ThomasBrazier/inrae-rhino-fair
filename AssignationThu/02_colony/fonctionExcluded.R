...
# Les femelles nees en 2017 ne sont pas prises en compte !!!!
      ...
      # Exclusion des femelles non juveniles d'une autre colonie nees en 2017
      temp=c(temp,as.vector(unique[which(unique$sexe=="F" & unique$idcol!=col & unique$yearWhenFirstCaptur==2017 & unique$ageWhenFirstCaptur!="Juv"),1]))
      itself=which(temp==as.vector(unique[index[i],1]))
      if (length(itself)==2) temp=temp[-itself[2]]
