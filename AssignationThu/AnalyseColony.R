...
maxDist=100 # distance maximale de la distribution
...
for (it in 1:nit) {
  ...
  coordCol = read.table("coordThu.txt", h = T)
  ...
  for (i in 1:nrow(dyadsH0)) {
    ...
    dyadsH0$distance[i] = distCosine(cbind(coordCol$Long[which(coordCol$Colony ==
                                                                 as.character(dyadsH0$offspring[i]))], coordCol$Lat[which(coordCol$Colony ==
                                                                                                                            as.character(dyadsH0$offspring[i]))]),
                                     cbind(coordCol$Long[which(coordCol$Colony == as.character(dyadsH0$father[i]))], coordCol$Lat[which(coordCol$Colony ==
...
dyadsObs=read.table("Paternities.txt",h=T)
...
for (i in 1:nrow(dyadsObs)) {
  # dyadsH0$distance[i]=acos(sin(coordCol$Lat[which(coordCol$Colonie==as.character(dyadsH0$offspring[i]))])*sin(coordCol$Lat[which(coordCol$Colonie==as.character(dyadsH0$father[i]))])+cos(coordCol$Lat[which(coordCol$Colonie==as.character(dyadsH0$offspring[i]))])*cos(coordCol$Lat[which(coordCol$Colonie==as.character(dyadsH0$father[i]))])*cos(coordCol$Long[which(coordCol$Colonie==as.character(dyadsH0$father[i]))]-coordCol$Long[which(coordCol$Colonie==as.character(dyadsH0$offspring[i]))]))*6371
  # create distance matrix
  dyadsObs$distance[i]=distCosine(cbind(coordCol$Long[which(coordCol$Colony==as.character(dyadsObs$offspring[i]))],coordCol$Lat[which(coordCol$Colony==as.character(dyadsObs$offspring[i]))]),
                                 cbind(coordCol$Long[which(coordCol$Colony==as.character(dyadsObs$father[i]))],coordCol$Lat[which(coordCol$Colony==as.character(dyadsObs$father[i]))]),
                                 r=6378137)/1000
}
