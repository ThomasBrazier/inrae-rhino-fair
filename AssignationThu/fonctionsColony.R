...
# ASSEMBLAGE DES DEUX JEUX DE DONNEES
#---------------------------------
...
#---------------------------------#
# CONSTRUCTION DES RESULTATS
# ET EXCLUSION DES FAUX POSITIFS
#---------------------------------
...
#---------------------------------#
# DISTRIBUTION DU NOMBRE DE RUNS
# POUR LESQUELS CHAQUE PERE EST TROUVE
#---------------------------------
...
# 6/ Sexe juvenile
# 7+/ Where was sampled the individual
...
describeResults=function(x=paternity){
  resultsWithInfo=data.frame(offspring=x[,1],father=x[,2],colonyOffsp=rep(NA,nrow(x)),colonyFather=rep(NA,nrow(x)),
                             ...
                             sexOffsp=rep(NA,nrow(x)),y2015_1=rep(NA,nrow(x)),y2015_2=rep(NA,nrow(x)),
                             y2016_1=rep(NA,nrow(x)),y2016_2=rep(NA,nrow(x)),
                             y2017_1=rep(NA,nrow(x)),y2017_2=rep(NA,nrow(x)))
  # Colonie de l'offsp
  info=read.table("uniqueGenotypesWithInfo.txt",h=T)
...
#-----------------------------#
# NOMBRE MOYEN DE JUVENILES PAR PARENT
#-----------------------------
Rsuccess=function(){
