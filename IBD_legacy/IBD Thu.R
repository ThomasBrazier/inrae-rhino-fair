...
if (Sys.info()["sysname"]=="Darwin") {
  setwd("/Volumes/Samsung_T5/INRA RHINO/R/IBD")
  wd="/Volumes/Samsung_T5/INRA RHINO/R/IBD"
  ...
  if (Sys.info()["sysname"]=="Windows"){
    setwd("E:/INRA RHINO/R/IBD")
    wd="E:/INRA RHINO/R/IBD"
...
allel=read.table("IBD/uniqueGenotypesWithInfoThu.txt",h=T)
...
coordCol=read.table("IBD/coordThu.txt",h=T)
...
coord=data.frame(Colonie=rep(NA,ncol(matGen)),Long=rep(NA,ncol(matGen)),Lat=rep(NA,ncol(matGen)))
coord$Colonie=lvl
...
# PLOT IBD PATTERN
dfThu=data.frame(Distance=as.vector(matGeo[lower.tri(matGeo,diag=TRUE)]),GeneDistance=as.vector(matGen[lower.tri(matGen,diag=TRUE)]))

##### KEEPING ONLY DISTANCES INFERIOR TO 30KM (SHORT SCALE)
dfThu=dfThu[which(dfThu$Distance<log(30) & dfThu$GeneDistance<0.2),]

plot(dfThu)
modThu=lm(GeneDistance~Distance,data=dfThu)
abline(lm(GeneDistance~Distance,data=dfThu))
modThu$coefficients[1]
modThu$coefficients[2]

save(dfThu,file="IBD/IBDThu.Rdata")
save(modThu,file="IBD/modIBDThu.Rdata")

#-------------------------------
# GRAPH GGPLOT OF IBD PATTERN (13 colonies)

# export 1800x1300
ggplot(data=dfThu, aes(x=Distance, y=GeneDistance)) +
  geom_point(size=2)+
  geom_abline(intercept=modThu$coefficients[1],slope=modThu$coefficients[2],size=2) +
