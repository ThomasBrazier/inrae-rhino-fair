...
I tag each step with a [x] if it successfully runs (without raising an error code) and [0] if it is not recommended to run it.
...
These scripts are mostly exploratory and most of it is not presented in the manuscript.
...
In addition these tables can be useful:

```
Tables/MigrantFathersPic.txt # The ancestry and natal colony of each father in Picardy
Tables/MigrantFathersThu.txt # The ancestry and natal colony of each father in Thuringia
Tables/ParamPic GAMETIC.txt # Parameters of the best fit dispersal kernel function on gametic distances
Tables/ParamPic.txt # Parameters of the best fit dispersal kernel function on mating distances
Tables/paramPicKernel GAMETIC.txt  # Model comparison for dispersal kernel functions on gametic distances
Tables/paramPicKernel.txt  # Model comparison for dispersal kernel functions on mating distances
Tables/ParamThu GAMETIC.txt # Parameters of the best fit dispersal kernel function on gametic distances
Tables/ParamThu.txt # Parameters of the best fit dispersal kernel function on mating distances
Tables/ParamThuKernel GAMETIC.txt  # Model comparison for dispersal kernel functions on gametic distances
Tables/ParamThuKernel.txt  # Model comparison for dispersal kernel functions on mating distances
Tables/table_masterbayes.tsv # Dispersal parameters estimated by masterBayes
```
